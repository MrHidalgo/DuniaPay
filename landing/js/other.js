  ...
  * ============================================= */
  ...
  $('body').on('click', function (ev) {
    ...
    if (!$(ev.target).closest(className).length) {
      ...
      $('.currency__box-dropdown').slideUp(300);
  ...
  var initTransferDropDown = function initTransferDropDown() {
    ...
    $('.transfer__box-dropdown li').on('click', function (ev) {
      var elem = $(ev.currentTarget),
        parentElem = elem.closest('.transfer__box-drop'),
        insertNode = parentElem.find('[transfer-btn-js]'),
        countryName = elem.find('p').text(),
        imgSrc = elem.find('img').attr('src');
      //var currencyCode = countries[countryName];
      $('.where').attr('value', countryName);
      //insertNode.find('input').html(currencyCode);
  ...
  var initCurrencyDropDown = function initCurrencyDropDown() {
    $('[currency-btn-js]').on('click', function (ev) {
      var elem = $(ev.currentTarget),
        parentElem = elem.closest('.transfer__box-drop');

      $('.currency__box-dropdown').slideUp(300);
      parentElem.find('.currency__box-dropdown').slideToggle(300);
    });
    $('.currency__box-dropdown li').on('click', function (ev) {
      var elem = $(ev.currentTarget),
        parentElem = elem.closest('.transfer__box-drop'),
        insertNode = parentElem.find('[currency-btn-js]'),
        currencyName = elem.find('p').text();
      //imgSrc = elem.find('img').attr('src');
      var currencyCode = countries[currencyName];
  ...
  var initRevertConverter = function initRevertConverter() {
    $('.shuffle').on('click', function (ev) {
      var elem = $(ev.currentTarget);
      elem.addClass('animated bounce delay-1s');
      $('.currency__box-dropdown').slideUp(300);
      parentElem.find('.currency__box-dropdown').slideToggle(300);
    });
    $('.currency__box-dropdown li').on('click', function (ev) {
